...
| 78 | `[~]` | Integration tests for `/api/cron/*` (mocked DB + platform APIs + notifications) — _Mongo integration harness built: `mongodb-memory-server` + `vitest.integration.config.ts` + `tests/integration/setup.ts` + CI `integration` job; first DB-model integration tests landed (`tests/integration/db.integration.test.ts`: AIUsage, PendingConnection). **Remaining**: cron-route tests with mocked platform adapters + notifications._ | `tests/integration/`, `app/api/cron/__tests__/` |
