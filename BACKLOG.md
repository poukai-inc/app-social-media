...
| 78 | `[~]` | Integration tests for `/api/cron/*` (mocked DB + platform APIs + notifications) — _Mongo integration harness built: `mongodb-memory-server` + `vitest.integration.config.ts` + `tests/integration/setup.ts` + CI `integration` job; DB-model tests (`db.integration.test.ts`: AIUsage, PendingConnection) + **publish cron route** (`cron-publish.integration.test.ts`: 401 auth, due-post publishes once + claim cleared, idempotent re-run — locks in #124 + #8) with mocked platform adapters. **Remaining**: engage / auto-generate / collect-metrics route tests + notification dispatch._ | `tests/integration/`, `app/api/cron/__tests__/` |
